...
public class ScriptsBuilder {
	...
	public void generateScripts(JhipsterConfiguration jconf, String jDirectory){
		...
		generateUnitTestDockerScript(jconf, jDirectory);
	...
	/**
	 * Generate script used to run tests on the configuration not generated by Docker and write it in test.sh.
	 * 
	...
	/**
	 * Generate script used to run tests on the configuration generated by Docker and write it in testDocker.sh.
	 * 
	 * @param jconf Configuration on which tests are run.
	 * @param jDirectory Directory where to write the script.
	 */
	...
	/**
	 * Generate the script to run the unit test(s) of the application not generated by Docker.
	 * 
	...
	/**
	 * Generate the script to run the unit test(s) of the application generated by Docker.
	 * 
	 * @param jconf JHipster configuration to test
	 * @param jDirectory Directory where the script must be generated
	 */
	private void generateUnitTestDockerScript(JhipsterConfiguration jconf, String jDirectory){
		String script = "#!/bin/bash\n\n";
		if (jconf.buildTool.equals("maven")) script += "./mvnw clean test >> testDocker.log 2>&1\n";
		else script += "./gradlew clean test >> testDocker.log 2>&1\n";
		// KarmaJS is provided by default
		script += "gulp test >> testDockerKarmaJS.log 2>&1\n";
		Files.writeStringIntoFile(getjDirectory(jDirectory)+"unitTestDocker.sh", script);
	}	
